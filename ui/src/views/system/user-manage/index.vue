<template>
  <div class="user-manage p-16-24">
    <h2 class="mb-16">{{ $t('views.userManage.title') }}</h2>
    <el-card class="user-manage__card" style="--el-card-padding: 0">
      <div class="user-manage__body p-24">
        <div class="user-manage__toolbar mb-16">
          <el-button
            type="primary"
            @click="createUser"
          ...
          <div class="complex-search user-manage__search">
            <el-select
        ...
        <div class="user-manage__table">
          <el-table :data="userTableData" v-loading="loading" height="100%">
          <el-table-column
            prop="nick_name"
          ...
          <el-table-column width="100" prop="is_active" :label="$t('common.status.label')">
            <template #default="{ row }">
              <div v-if="row.is_active" class="flex align-center">
                ...
                <span class="color-text-primary">{{ $t('common.status.enabled') }}</span>
              </div>
              <div v-else class="flex align-center">
                <AppIcon iconName="app-disabled" class="color-secondary mr-8"></AppIcon>
                <span class="color-text-primary">{{ $t('common.status.disabled') }}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column
...
const search_type_change = () => {
  search_form.value = { username: '', nick_name: '', email: '', is_active: null, source: '' }
}
...
function refresh(options?: { resetPage?: boolean; clearSearch?: boolean }) {
  if (options?.clearSearch) {
    search_type.value = 'username'
    search_form.value = {
      username: '',
      nick_name: '',
      email: '',
...
.user-manage__card {
  height: calc(var(--app-main-height) + 10px);
}
